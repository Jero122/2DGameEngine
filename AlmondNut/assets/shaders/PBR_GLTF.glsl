...
{
	float mipCount = float(textureQueryLevels(prefilterMap));
	...
	vec3 specularLight = textureLod(prefilterMap, reflection.xyz * cm, lod).rgb;
...
vec3 calculatePBRLightContribution( inout PBRInfo pbrInputs, DirLight dirLight )
{
	vec3 n = pbrInputs.n;
	vec3 v = pbrInputs.v;
	vec3 l = normalize(dirLight.direction);	// Vector from surface point to light
	vec3 h = normalize(l+v);				// Half vector between both l and v
	vec3 lightColor = dirLight.diffuse * dirLight.ambient;
	...
	vec3 specContrib = F * G * D / (4.0 * NdotL * NdotV) * dirLight.specular;
...
void main()
{
	float Kao = texture(material.aoRoughnessMetallicMap, TexCoords).r;
	vec4 Ke  = texture(material.emissiveMap, TexCoords);
	vec4 Kd  = texture(material.albedoMap, TexCoords);
	vec2 MeR = texture(material.aoRoughnessMetallicMap, TexCoords).yz;
	...
	vec3 normalSample = texture(material.normalMap, TexCoords).xyz;

	// normal mapping
	n = perturbNormal(n, normalize(viewPos.xyz - FragPos), normalSample, TexCoords);

	vec4 mrSample = texture(material.aoRoughnessMetallicMap, TexCoords);

	PBRInfo pbrInputs;
	Ke.rgb = SRGBtoLINEAR(Ke).rgb;
	// image-based lighting
	vec3 color = calculatePBRInputsMetallicRoughness(Kd, n, viewPos.xyz, FragPos, mrSample, pbrInputs);

	// one hardcoded light source
	 if(dirLight.enabled)
    {
        // phase 1: Directional lighting
		color += calculatePBRLightContribution( pbrInputs,dirLight);
    }
	// ambient occlusion
	color = color * Kao;
	// emissive
